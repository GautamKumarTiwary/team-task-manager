...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [projRes, usersRes] = await Promise.all([
        axios.get('https://team-task-manager-k4ph.onrender.com/api/projects', config),
        axios.get('https://team-task-manager-k4ph.onrender.com/api/auth/users', config)
      ]);
      setProjects(projRes.data);
      setUsers(usersRes.data.filter(u => u.role === 'Member'));
      
      // Fetch tasks for all projects
      const tasksPromises = projRes.data.map(p => axios.get(`https://team-task-manager-k4ph.onrender.com/api/tasks/project/${p._id}`, config));
  ...
  const handleCreateProject = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.post('https://team-task-manager-k4ph.onrender.com/api/projects', newProject, config);
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.post('https://team-task-manager-k4ph.onrender.com/api/tasks', newTask, config);
