...
const MemberDashboard = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const { data } = await axios.get('https://team-task-manager-k4ph.onrender.com/api/tasks/user', config);
      setTasks(data);
    } catch (error) {
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.put(`https://team-task-manager-k4ph.onrender.com/api/tasks/${taskId}/status`, { status: newStatus }, config);
      fetchTasks();
    } catch (error) {
